...
foi_path <- "data/foi"
foi_files <- list.files(path = foi_path, full.names = TRUE)[3] # 3 hard coded here. Remove
...
postcode_lookup <- read_csv(
  ...
  select(pcd7, pcd8, lad_code = ladcd)
...
foi_combined <- foi_data |>
  ...
  mutate(lad_code = coalesce(lad_code_7, lad_code_8)) |>
