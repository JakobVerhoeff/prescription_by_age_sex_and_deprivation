...
imd_pops <- read_csv("data/popsbyimdengland20012019.csv")[, 1:23] %>%
  ...
  mutate(
    age_band = dplyr::recode(age,
      "<1" = "0-5", "01-04" = "0-5",
      "05-09" = "6-10",
      "10-14" = "11-20", "15-19" = "11-20",
      "20-24" = "21-30", "25-29" = "21-30",
      "30-34" = "31-40", "35-39" = "31-40",
      "40-44" = "41-50", "45-49" = "41-50",
      "50-54" = "51-60", "55-59" = "51-60",
      "60-64" = "61-70", "65-69" = "61-70",
      "70-74" = "71-80", "75-79" = "71-80",
      "80-84" = "81-90", "85-89" = "81-90",
      "90+" = "91-100"
    ),
    gender = recode(gender, "males" = "man", "females" = "woman")
  )

age_band_levels <- c("0-5", "6-10", "11-20", "21-30", "31-40", "41-50",
...
ggplot(ons_popn, aes(x = population)) +
  ...
  labs(title = "Population Distribution by LAD",
       x = "Population", y = "Count of LADs") +
  theme_minimal()
...
ggplot(imd_pops_sex, aes(x = split10, y = popn, fill = age_band)) +
  ...
  labs(x = "IMD Decile (Q1 = most deprived)",
       y = "Population proportion", fill = "Age Band") +
  theme_minimal() +
...
ggplot(imd_pops_sex, aes(x = split10, y = popn, fill = age_band)) +
  ...
  facet_wrap(~gender) +
  labs(x = "IMD Decile (Q1 = most deprived)",
       y = "Population size", fill = "Age Band") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(imd_overlay,
  ...
  coord_flip() +
  scale_y_continuous(labels = function(x) paste0(abs(x) * 100, "%")) +
  scale_fill_manual(values = c("woman - Q1"  = "#E76F51",
                               "woman - Q5" = "#F4A896",
                               "man - Q1"    = "#264653",
                               "man - Q5"   = "#7BA7B5")) +
...
imd_overlay_abs <- imd_pops_sex %>%
  filter(imd %in% c(1, 5)) %>%
  mutate(popn = ifelse(gender == "man", -popn, popn),
...
sex_props <- imd_pops_sex %>%
  ...
  mutate(pct_man = round(100 * man / (man + woman), 2),
         total = man + woman)
...
prescribing <- combined_data |>
  ...
  mutate(
    ...
    imd_quintile = factor(imd_quintile, levels = c("Q1", "Q2", "Q3", "Q4", "Q5"))
  )
...
overall_by_imd <- prescribing |>
  ...
  left_join(
    pop_denominators |> group_by(imd_quintile, year) |>
...
ggplot(overall_by_imd, aes(x = imd_quintile, y = rate_per_1000)) +
  geom_col() +
  facet_wrap(~year) +
  labs(x = "IMD Quintile (Q1 = most deprived)",
...
by_family_imd <- prescribing |>
  ...
  summarise(total_items = sum(total_items, na.rm = TRUE), .groups = "drop") |>
  left_join(
    pop_denominators |> group_by(imd_quintile) |>
      summarise(total_pop = sum(pop_a_s, na.rm = TRUE), .groups = "drop"),
    by = "imd_quintile"
  ) |>
  mutate(rate_per_1000 = total_items / total_pop * 1000)

ggplot(by_family_imd, aes(x = imd_quintile, y = rate_per_1000, fill = antibiotic_group)) +
  geom_col() +
...
by_family_imd_sex <- prescribing |>
  ...
  summarise(total_items = sum(total_items, na.rm = TRUE), .groups = "drop") |>
  left_join(
    pop_denominators |> group_by(imd_quintile, gender) |>
      summarise(total_pop = sum(pop_a_s, na.rm = TRUE), .groups = "drop"),
    by = c("imd_quintile", "gender")
  ) |>
  mutate(rate_per_1000 = total_items / total_pop * 1000)

ggplot(by_family_imd_sex, aes(x = imd_quintile, y = rate_per_1000,
...
by_family_age_imd <- prescribing |>
  ...
  summarise(total_items = sum(total_items, na.rm = TRUE), .groups = "drop") |>
  left_join(
    pop_denominators |> group_by(age_band, imd_quintile) |>
      summarise(total_pop = sum(pop_a_s, na.rm = TRUE), .groups = "drop"),
    by = c("age_band", "imd_quintile")
  ) |>
  mutate(rate_per_1000 = total_items / total_pop * 1000)

ggplot(by_family_age_imd,
       aes(x = age_band, y = rate_per_1000,
           colour = imd_quintile, group = imd_quintile)) +
  geom_line() +
  geom_point(size = 1) +
  facet_wrap(~antibiotic_group, scales = "free_y") +
...
by_family_age_imd_sex <- prescribing |>
  ...
  summarise(total_items = sum(total_items, na.rm = TRUE), .groups = "drop") |>
  left_join(
    pop_denominators |> group_by(age_band, imd_quintile, gender) |>
      summarise(total_pop = sum(pop_a_s, na.rm = TRUE), .groups = "drop"),
    by = c("age_band", "imd_quintile", "gender")
  ) |>
  mutate(rate_per_1000 = total_items / total_pop * 1000)

ggplot(by_family_age_imd_sex,
       aes(x = age_band, y = rate_per_1000,
           colour = imd_quintile, linetype = gender,
...
ggsave("plots/prescribing_by_family_age_IMD_sex_panels.pdf", width = 16, height = 12)

ggplot(by_family_age_imd_sex %>% filter(imd_quintile %in% c("Q1","Q5")),
       aes(x = age_band, y = rate_per_1000,
           colour = imd_quintile,
           group = interaction(imd_quintile, antibiotic_group, gender))) +
  geom_line() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7),
        strip.text.y = element_text(angle = 0, hjust = 0))
ggsave("plots/prescribing_by_family_age_IMD_sex_group_panels.pdf", width = 16, height = 12)

# Selected antibiotic families only
key_families <- c("Penicillins", "Macrolides", "Tetracyclines", "UTIs",
                  "Quinolones", "Ceph's", "MTO")
ggplot(by_family_age_imd_sex %>% 
         filter(antibiotic_group %in% key_families,
                imd_quintile %in% c("Q1","Q5")),
       aes(x = age_band, y = rate_per_1000,
           colour = imd_quintile,
           group = interaction(imd_quintile, antibiotic_group, gender))) +
  geom_line() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7),
        strip.text.y = element_text(angle = 0, hjust = 0))
ggsave("plots/prescribing_by_family_age_IMD_sex_key_families.pdf", width = 16, height = 12)

# Selected age bands only
ggplot(by_family_age_imd_sex %>% filter(age_band %in% c("11-20","21-30","31-40","41-50","51-60")),
       aes(x = age_band, y = rate_per_1000,
           colour = imd_quintile,
           group = interaction(imd_quintile, antibiotic_group, gender))) +
  geom_line() +
...
by_family_age_imd_sex_yr <- prescribing |>
  ...
  summarise(total_items = sum(total_items, na.rm = TRUE), .groups = "drop") |>
  left_join(
    pop_denominators |> group_by(age_band, imd_quintile, gender, year) |>
      summarise(total_pop = sum(pop_a_s, na.rm = TRUE), .groups = "drop"),
...
for (yr in sort(unique(by_family_age_imd_sex_yr$year))) {
  ggplot(by_family_age_imd_sex_yr |> filter(year == yr),
         aes(x = age_band, y = rate_per_1000,
             colour = imd_quintile, linetype = gender,
             group = interaction(imd_quintile, gender))) +
    geom_line() +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7))
  ggsave(paste0("plots/prescribing_by_family_age_IMD_sex_", yr, ".pdf"),
         width = 14, height = 8)
}
