...
data_totals <- data %>% group_by(GENDER, AGE_BAND, quintile) %>%
  summarise(total_patients_all = sum(total_patients, na.rm = TRUE),
            ...
            items4_per_patient_all_all = items4_total / total_patients_all)
...
g1a <- ggplot(data_totals %>% filter(!is.na(AGE_BAND) & quintile != 'NA', GENDER == "Female"), 
              aes(x = AGE_BAND, y = (items_per_patient_all + items4_per_patient_all) / 2, fill = quintile)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7)) +
  geom_errorbar(aes(ymin = items_per_patient_all, ymax = items4_per_patient_all),
                width = 0.2, position = position_dodge(width = 0.7)) +
...
# ### EDIT FOIDATA ##########################################
# ###########################################################
# foidata <- merge(foidata_orig, deprivationdata[, c("PRACTICE_CODE", "quintile")], 
#                  by = "PRACTICE_CODE", all.x = TRUE)
# foidata$ITEMS<-as.numeric(foidata$ITEMS) # Use items not patient number 
# foidata$ITEMS[is.na(foidata$ITEMS)] <- 1 # I convert the NA's into 1's. ASSUMPTION!
# foidata <- foidata[foidata$GENDER %in% c("Male", "Female"), ] # remove those with no gender
# foidata<- foidata %>% 
#   select(-UNIQUE_PATIENT_COUNT,-FINANCIAL_YEAR,-BNF_CHEMICAL_SUBSTANCE_CODE) # remove those things we won't use
# 
# # Summarise total items 
# foidata <- foidata %>%
...
# ggsave("plots/figure1a.pdf")
# 
# g1a <- ggplot(foidata_combined %>% filter(!is.na(AGE_BAND) & quintile != 'NA', GENDER == "Female"), 
#               aes(x = AGE_BAND, y = (items_per_patient + items_per_patient_na4) / 2, fill = quintile)) +
#   geom_bar(stat = "identity", position = position_dodge(width = 0.7)) +
#   geom_errorbar(aes(ymin = items_per_patient, ymax = items_per_patient_na4),
#                 width = 0.2, position = position_dodge(width = 0.7)) +
