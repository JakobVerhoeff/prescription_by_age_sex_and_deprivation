...
model_data <- combined_data |>
  ...
  group_by(lad_code, imd_quintile, age_band, gender) |>
  summarise(
...
m1 <- glmmTMB(
  total_items ~ 1 + offset(log(pop)) + (1 | lad_code),
  family = nbinom2, data = model_data
)

# 2. + IMD quintile
m2 <- glmmTMB(
  total_items ~ imd_quintile + offset(log(pop)) + (1 | lad_code),
  family = nbinom2, data = model_data
)

# 3. + IMD + age
m3 <- glmmTMB(
  total_items ~ imd_quintile + age_band + offset(log(pop)) + (1 | lad_code),
  family = nbinom2, data = model_data
)

# 4. + IMD + sex
m4 <- glmmTMB(
  total_items ~ imd_quintile + gender + offset(log(pop)) + (1 | lad_code),
  family = nbinom2, data = model_data
)

# 5. + IMD + age + sex + age*sex
m5 <- glmmTMB(
  total_items ~ imd_quintile + age_band * gender + offset(log(pop)) + (1 | lad_code),
  family = nbinom2, data = model_data
)

# 6. + IMD + age + sex + age*sex + IMD*age + IMD*sex
m6 <- glmmTMB(
  total_items ~ imd_quintile * age_band + imd_quintile * gender + offset(log(pop)) + (1 | lad_code),
  family = nbinom2, data = model_data
)

# AIC comparison table
aic_table <- aictab(
  cand.set  = list(m1, m2, m3, m4, m5, m6),
  modnames  = c("null", "IMD", "IMD+age", "IMD+sex", "IMD+age+sex+age:sex", "IMD+age+sex+IMD:age+IMD:sex")
)
...
# Not sure what this says - remove model 6?
